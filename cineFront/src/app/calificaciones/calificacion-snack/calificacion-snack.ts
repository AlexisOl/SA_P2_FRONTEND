...
import {
  CrearCalificacionSnackDTO, MAX_CARACTERES_COMENTARIO, PUNTUACION_MAXIMA,
  PUNTUACION_MINIMA,
...
import {CalificacionSnackService} from '@/services/calificacion-snack.service';
import {ConfirmationService, MessageService} from 'primeng/api';

@Component({
  selector: 'app-calificacion-snack',
  imports: [
    CommonModule,
    ReactiveFormsModule,
    FormsModule,
  ...
  providers: [MessageService, ConfirmationService, CalificacionSnackService],
  templateUrl: './calificacion-snack.html',
  styleUrl: './calificacion-snack.scss',
})
export class CalificacionSnack implements OnInit {

  calificaciones: ResponseCalificacionSnackDTO[] = [];
  calificacionSeleccionada: ResponseCalificacionSnackDTO | null = null;
  ...
  constructor(
    private calificacionService: CalificacionSnackService,
  ...
  crearCalificacion(): void {
    ...
    const nuevaCalificacion: CrearCalificacionSnackDTO = {
      usuarioId: this.calificacionForm.value.usuarioId,
      snackId: this.calificacionForm.value.snackId,
    ...
    this.calificacionService.crearCalificacion(nuevaCalificacion).subscribe({
      next: (calificacion) => {
        ...
        this.cargarCalificaciones();
        this.cargarPromedioCalificacion(calificacion.snackId);
        this.loading = false;
      },
      error: (error) => {
  ...
  cargarCalificacionesPorSnack(snackId: string): void {
    this.loading = true;
    this.calificacionService.listarCalificacionesPorSnack(snackId).subscribe({
      next: (calificaciones) => {
        this.calificaciones = calificaciones;
        this.totalCalificaciones = calificaciones.length;
        this.calcularDistribucionEstrellas();
        this.cargarPromedioCalificacion(snackId);
        this.loading = false;
      },
      error: (error) => {
        console.error('Error al cargar calificaciones:', error);
  ...
  verDetalleCalificacion(calificacion: ResponseCalificacionSnackDTO): void {
    this.calificacionSeleccionada = calificacion;
    this.displayDetalleDialog = true;
  }


  cargarPromedioCalificacion(snackId: string): void {
    this.loadingPromedio = true;
    this.calificacionService.obtenerPromedioCalificacion(snackId).subscribe({
      next: (response) => {
