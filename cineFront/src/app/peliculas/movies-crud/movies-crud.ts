...
@Component({
  ...
  imports: [
    ...
    InputTextModule, TextareaModule, TagModule, RippleModule, IconFieldModule, InputIconModule,
    SelectModule, DatePickerModule, ToggleSwitchModule, InputNumberModule, SelectModule, DatePickerModule, ToggleSwitchModule, InputNumberModule, CheckboxModule
  ],
  template: `
  <p-toolbar styleClass="mb-6">
    <ng-template #start>
      ...
      <p-button
        [label]="bulkLabel()" [icon]="bulkIcon()" [severity]="bulkSeverity()" outlined
        (onClick)="toggleSelected()" [disabled]="!selected?.length" />
    </ng-template>

    <ng-template #end>
      <p-iconfield>
        <p-inputicon styleClass="pi pi-search" />
        <input pInputText type="text" (input)="onGlobal(dt, $event)" placeholder="Buscar título..." />
      </p-iconfield>
    </ng-template>
  </p-toolbar>
  ...
  >
    ...
    <ng-template #body let-m>
      <tr>
        ...
        <td>
          <img [src]="firstPoster(m)" alt="poster" style="width:56px;height:56px;object-fit:cover" class="rounded" />
        </td>
        <td>{{ m.titulo }}</td>
        ...
        <td>
          <p-tag [value]="m.activa ? 'ACTIVA' : 'INACTIVA'" [severity]="m.activa ? 'success' : 'danger'"/>
        </td>
        <td class="text-right">
          <p-button icon="pi pi-pencil" class="mr-2" [rounded]="true" [outlined]="true" (click)="edit(m)" />
          <p-button [icon]="m.activa ? 'pi pi-ban' : 'pi pi-check'"
          ...
          <p-button icon="pi pi-trash" severity="danger" [rounded]="true" [outlined]="true"
                    (click)="remove(m)" />
          <p-button label="Categoría" icon="pi pi-tags" [outlined]="true" class="mr-2"
  ...
  <!-- Dialog Crear/Editar -->
  <!-- Dialog Crear/Editar -->
<p-dialog [(visible)]="dialog" [style]="{ width: '720px' }" [modal]="true"
          header="{{ editing ? 'Editar' : 'Nueva' }} película">
  <ng-template #content>
    <!-- p-fluid aplica 100% width a inputs compatibles -->
    <div class="grid formgrid p-fluid" style="row-gap:1rem; column-gap:1rem">
      ...
      <div class="col-12 md:col-6">
        ...
        <input pInputText class="w-full" [(ngModel)]="form.titulo" required />
      ...
      <div class="col-12">
        ...
        <textarea pTextarea class="w-full" rows="3" [(ngModel)]="form.sinopsis"></textarea>
      ...
      <div class="col-12">
        ...
        <input pInputText class="w-full" [(ngModel)]="castInput" placeholder="Ej: Nombre1, Nombre2" />
...
<p-dialog [(visible)]="dialogCategorias" [style]="{ width: '520px' }" [modal]="true" header="Definir categoría">
  <ng-template #content>
    <div class="pt-2">
  <label class="block font-bold mb-2">Selecciona categorías</label>

  <div *ngIf="!loadingCategorias; else catsLoading" class="flex flex-col gap-2">
    <div *ngIf="categoriasTodas?.length; else noCats">
      <!-- Checkboxes con binding a selectedCategoriaIds -->
      <div *ngFor="let c of categoriasTodas" class="flex items-center gap-2 border rounded p-2">
        <p-checkbox
          name="cats"
          [value]="c.id"
        ...
        <span>{{ c.nombre }}</span>
        <p-tag value="Activa" severity="success" *ngIf="c.activa"></p-tag>
      </div>

      <div class="flex justify-end mt-3 gap-2">
        <p-button label="Aplicar cambios" icon="pi pi-check" (click)="applyCategorias()" />
      </div>
    </div>
  ...
  <ng-template #footer>
    <p-button label="Cerrar" icon="pi pi-check" (click)="dialogCategorias=false" />
  </ng-template>
</p-dialog>
...
_prevCategoriaIds: string[] = [];
  ...
  edit(row: Movie) {
    this.form = { ...row };
    this.fechaEstrenoModel = row.fechaEstreno ? new Date(row.fechaEstreno) : null;
  ...
  hideDialog() { this.dialog = false; this.submitted = false; }

  save() {
    this.submitted = true;
    if (!this.form.titulo || !this.form.sinopsis || !this.form.duracion || !this.form.clasificacion || !this.fechaEstrenoModel) return;
    ...
    const payload: Movie = {
      ...
      cast: this.castInput ? this.castInput.split(',').map(s => s.trim()).filter(Boolean) : [],
    ...
    const req = payload.id ? this.svc.update(payload.id, payload) : this.svc.create(payload);
    req.subscribe({
      next: () => {
        this.toast.add({ severity: 'success', summary: 'OK', detail: 'Película guardada', life: 2500 });
        this.dialog = false;
        this.fetch();
      }
    });
  ...
  toggleOne(row: Movie) {
    ...
    this.confirm.confirm({
      ...
      accept: () => {
        const req = row.activa ? this.svc.desactivar(row.id!) : this.svc.activar(row.id!);
        req.subscribe({
          next: () => {
            this.toast.add({ severity: 'success', summary: 'OK', detail: `Película ${accion}a`, life: 2200 });
            this.fetch();
          }
        });
      }
  ...
  toggleSelected() {
    ...
    const msg = activas && inactivas
      ? `Se activarán ${inactivas} y desactivarán ${activas} películas. ¿Continuar?`
      : activas ? `Se desactivarán ${activas} películas. ¿Continuar?`
                : `Se activarán ${inactivas} películas. ¿Continuar?`;

    this.confirm.confirm({
      message: msg, header: 'Confirmar', icon: 'pi pi-exclamation-triangle',
      accept: () => {
        ...
        this.selected!.forEach(m => {
          ...
          req.subscribe({
            next: () => { done++; if (done === total) { this.toast.add({ severity:'success', summary:'OK', detail:'Cambio aplicado', life:2200 }); this.selected=null; this.fetch(); } }
          });
        });
      }
    });
  }
  bulkLabel() { if (!this.selected?.length) return 'Activar/Desactivar'; const a=this.selected.filter(x=>x.activa).length; const i=this.selected.length-a; return a&&i?'Aplicar cambios':a?'Desactivar seleccionadas':'Activar seleccionadas'; }
  bulkIcon()  { if (!this.selected?.length) return 'pi pi-refresh'; const a=this.selected.filter(x=>x.activa).length; const i=this.selected.length-a; return a&&i?'pi pi-refresh':a?'pi pi-ban':'pi pi-check'; }
  bulkSeverity(): ButtonSeverity { if (!this.selected?.length) return 'secondary'; const a=this.selected.filter(x=>x.activa).length; const i=this.selected.length-a; return a&&i?'secondary':a?'warn':'success'; }
    ...
    this.confirm.confirm({
      ...
      accept: () => {
        this.svc.remove(row.id!).subscribe({
          next: () => { this.toast.add({ severity:'success', summary:'OK', detail:'Película eliminada', life:2200 }); this.fetch(); }
        });
      }
    });
  }
...
openCategorias(m: Movie) {
  ...
  forkJoin({
    ...
    next: ({ asignadas, todas }) => {
      ...
      this.categoriasTodas = (todas ?? []).map(t => ({
        id: (t as any).id ?? (t as any).categoriaId ?? (t as any).uuid ?? null,
        nombre: (t as any).nombre ?? (t as any).name ?? '',
        activa: !!(t as any).activa
      })).filter(x => x.activa);

      // Mapa nombre->id para resolver casos donde asignadas no traen id
      const byNombre = new Map(this.categoriasTodas.map(c => [c.nombre.trim().toLowerCase(), c.id]));
      ...
      this.selectedCategoriaIds = this.categoriasAsignadas
        .map(a => a.id || byNombre.get(a.nombre.trim().toLowerCase()) || null)
    ...
    error: () => {
      this.loadingCategorias = false;
      this.toast.add({ severity: 'error', summary: 'Error', detail: 'Error al cargar categorías', life: 2500 });
    }
  });
}
...
applyCategorias() {
  ...
  const toAdd: string[] = [];
  now.forEach(id => { if (!prev.has(id)) toAdd.push(id); });

  // A quitar: en "prev" pero no en "now"
  const toRemove: string[] = [];
  prev.forEach(id => { if (!now.has(id)) toRemove.push(id); });

  if (!toAdd.length && !toRemove.length) {
    this.toast.add({ severity:'info', summary:'Sin cambios', detail:'No hay cambios en categorías', life:2000 });
    return;
  }

  // Ejecutar en paralelo
  const attaches = toAdd.map(id => this.svc.attachCategory(this.currentMovieId!, id));
  const detaches = toRemove.map(id => this.svc.detachCategory(this.currentMovieId!, id));

  forkJoin([ ...(attaches.length ? [forkJoin(attaches)] : []),
             ...(detaches.length ? [forkJoin(detaches)] : []) ]).subscribe({
    next: () => {
      this.toast.add({ severity: 'success', summary: 'OK', detail: 'Categorías actualizadas', life: 2200 });
...
asignarCategoria() {
  ...
  this.svc.attachCategory(this.currentMovieId, this.categoriaSeleccionada).subscribe({
    next: () => {
      this.toast.add({ severity: 'success', summary: 'OK', detail: 'Categoría asignada', life: 2200 });
  ...
  this.confirm.confirm({
    ...
    accept: () => {
      this.svc.detachCategory(this.currentMovieId!, categoriaId).subscribe({
        next: () => {
          this.toast.add({ severity: 'success', summary: 'OK', detail: 'Categoría quitada', life: 2200 });
          this.categoriasAsignadas = this.categoriasAsignadas.filter(c => c.id !== categoriaId);
        }
      });
    }
  });
