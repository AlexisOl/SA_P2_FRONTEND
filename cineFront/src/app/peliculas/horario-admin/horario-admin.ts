...
// export interface MovieMin { id: string; titulo: string; }
...
//     InputTextModule, TagModule, RippleModule, SelectModule, DatePickerModule, ToggleSwitchModule, InputNumberModule
//   ],
//   template: `
...
//     </ng-template>
//     <ng-template #end>
//       <p-button icon="pi pi-plus" label="Nuevo horario" (onClick)="newHorario()" [disabled]="!sel.peliculaId || !sel.cinemaId || !salas.length"/>
//     </ng-template>
//   </p-toolbar>
...
//       <p-toggleswitch [(ngModel)]="soloActivos" (onChange)="load()"></p-toggleswitch><span>Solo activos</span>
//     </div>
//     <div class="col-12 md:col-3 flex gap-2">
//       <p-button label="Buscar" icon="pi pi-search" (click)="load()" />
//       <p-button label="Limpiar" icon="pi pi-eraser" severity="secondary" (click)="clearFilters()" />
//     </div>
//   </div>

//   <!-- Tabla -->
//   <p-table #dt [value]="rows()" [paginator]="true" [rows]="10" [rowHover]="true" [tableStyle]="{ 'min-width': '72rem' }">
//     <ng-template #header>
//       <tr>
...
//         this.peliculas = (list || []).map((m: any) => ({ id: m.id, titulo: m.titulo }));
//       }
//     });
...
//   // encadenados
//   onChangePelicula() { this.rows.set([]); }
...
//       next: (salas: any) => {
//         this.salas = (salas || []).map((s: any) => ({ id: s.id, nombre: s.nombre || s.name || `Sala ${s.id}` }));
//       }
//     });
//   }
...
//       this.toast.add({ severity:'info', summary:'Filtro', detail:'Selecciona una película', life:2000 });
//       return;
//     }
//     this.horarios.list({
...
//       this.toast.add({ severity:'warn', summary:'Campos', detail:'Selecciona película, cine y sala', life:2000 });
//       return;
//     }
...
//     this.form = { id: h.id, idioma: h.idioma, formato: h.formato, precio: h.precio ?? 0 };
...
//   save() {
//     const ok = this.sel.peliculaId && this.sel.cinemaId && this.sel.salaId && this.form.idioma && this.form.formato && this.formInicio && this.formFin;
...
//     const req = this.form.id ? this.horarios.update(this.form.id, payload) : this.horarios.create(payload);
//     req.subscribe({
//       next: () => {
